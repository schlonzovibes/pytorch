...
STUDY_STORAGE = "sqlite:///../optuna_maskrcnn.db"
...
# out. Re-enable "bf16-mixed" only after the run is stable.
PRECISION = "32-true"
...
# CHANGED: persistent workers across short-lived trainers caused
# "AssertionError: can only test a child process" during teardown.
...
def build_trainer(max_epochs, callbacks, enable_checkpointing=False, progress_bar=True):
    """All Trainer instances share these settings - logger and checkpointing
...
def measure_noise_floor(params, repeats=NOISE_REPEATS):
    ...
    for run in range(repeats):
        ...
        trainer = build_trainer(SEARCH_EPOCHS, callbacks=[], progress_bar=False)
...
if RUN_MODE == "noise":
    # Previous best configuration, used as the reference point
    measure_noise_floor(
        {
            "lr": 0.008,
            "momentum": 0.855,
            "weight_decay": 8.9e-05,
            "batch_size": 2,
        }
    )
    raise SystemExit(0)
...
checkpoint_callback = ModelCheckpoint(
    ...
    filename=CHECKPOINT_PREFIX + "epoch{epoch:02d}_valiou{val_iou:.4f}_valloss{val_loss:.4f}",
...
def prune_checkpoints(directory, prefix, pattern):
    ...
    for score, path in candidates[1:]:
        os.remove(path)
        print(f"Removed older checkpoint (val_iou {score:.4f}): {os.path.basename(path)}")
